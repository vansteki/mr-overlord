...
function CarClassNamePresent() {
    this.translateObject = function(trainInfo) {
        switch (trainInfo.CarClass) {
            case "1130":
                trainInfo.carClassName = "電車";
                break;
            case "1131":
                trainInfo.carClassName = "區間";
                break;
            case "1132":
                trainInfo.carClassName = "區間快";
                break;
            case "1100":
            case "1101":
            case "1108":
                trainInfo.carClassName = "自強";
                break;
            case "1102":
                trainInfo.carClassName = "太魯閣";
                break;
            case "1107":
                trainInfo.carClassName = "普悠瑪";
                break;
            case "1110":
            case "1114":
            case "1115":
                trainInfo.carClassName = "莒光";
                break;
            case "1120":
                trainInfo.carClassName = "復興";
                break;
            case "1140":
                trainInfo.carClassName = "普快車";
                break;
            case "1141":
                trainInfo.carClassName = "柴快車";
                break;
            case "1150":
                trainInfo.carClassName = "柴油車";
                break;
        }
        return trainInfo;
    };

    this.translateNumber = function(carClassNum) {
        var mapNameObj = {
            1100: "自強",
            1101: "自強",
            1102: "太魯閣",
        ...
        return mapNameObj[carClassNum];
    };
};

function TimeFilter() {
    this.now = function(timeTocompare) {
        return ( +new Date <= unixifyTime(timeTocompare)) ? 1:0;
    }
}
...
module.exports = function(robot) {
    ...
    robot.hear(/ri (\D+)+>+(\D+)/i, function(msg) {
        var argPart = msg.match[2].split(' ');
        msg.match[2] = argPart[0];
        var opt = _.partition(argPart.splice(1), function(v){
            return v.match(/-/);
        });
        var optObj = _.zipObject(opt[0],opt[1]);
        ...
        dataPrepare.on('done', function(msg) {
            ...
            finalList.forEach(function(trainInfo) {
                ...
                finalPairList.push(trainInfo);
            });

            var outPutPairList = [];
            finalPairList.forEach(function(trainInfo, key) {
                trainInfo = (new CarClassNamePresent).translateObject(trainInfo);
                outPutPairList.push(trainInfo.pair);
                outPutPairList[key].unixtime = unixifyTime(trainInfo.pair[0].ARRTime);
                outPutPairList[key].carClassName = trainInfo.carClassName;
                outPutPairList[key].carClassNum = trainInfo.CarClass;
                outPutPairList[key].trainId = trainInfo.Train;
            });
            outPutPairList = _.sortBy(outPutPairList, 'unixtime');

            var timeFilter = new TimeFilter();
            outPutPairList = _.without( outPutPairList.map(function(trainInfo) {
                if (optObj['-t'] == "now") {
                    if (timeFilter.now(trainInfo[0].ARRTime)) return trainInfo;
                } else {
                    return trainInfo;
                }
            }) , undefined);
            ...
            var info = {};
            info.header = "\n"+ fileDate + " 由 " + msg.match[1] + " 開往 " + msg.match[2] + " 的列車\n";
            info.header += "\n車次\t車種\t開車\t抵達\t\n";
            info.table = "";
            msg.send(info.header);
            outPutPairList.forEach(function(trainInfo) {
                info.table += trainInfo.trainId + "\t"
                    + trainInfo.carClassName + "\t"
                    + trainInfo[0].ARRTime.slice(0, -3) + "\t"
                    + trainInfo[1].DEPTime.slice(0,-3) + "\n"
            });
            msg.send(info.table);
        });
    });
